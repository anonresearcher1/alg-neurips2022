...
from src.client.client_per_fedavg import *
...
for iteration in range(args.rounds):
    ...
    total_num_data = sum([len(net_dataidx_map[idx]) for idx in idxs_users])
    weight_avg = [len(net_dataidx_map[idx]) / total_num_data for idx in idxs_users]
    w_glob = FedAvg(w_locals, weight_avg=weight_avg)
